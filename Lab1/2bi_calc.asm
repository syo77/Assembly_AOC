section .data
    ...
    erro_msg db 10, "Erro: Divisão por zero!", 10
             db "", 0
...
multiplicacao:
    call ler_numeros
    mov rax, rbx  ; Coloca num1 em RAX
    xor rdx, rdx  ; Zera RDX antes da multiplicação para evitar erros
    mul rcx       ; Multiplica RAX por RCX (resultado fica em RDX:RAX)
    mov rbx, rax  ; Move o resultado final para RBX (para exibição)
    jmp resultado
...
divisao:
    ...
    cmp rcx, 0
    je divisao_por_zero  ; Se rcx for zero, pular para mensagem de erro

    ; Preparação para a divisão
    mov rdx, 0      ; Zera o resto de divisão (rdx)
    mov rax, rbx    ; Coloca o dividendo (num1) em rax

    div rcx         ; Divide rax por rcx. O quociente vai para rax e o resto vai para rdx.

    mov rbx, rax    ; Armazena o resultado da divisão (quociente) em rbx
    jmp resultado

divisao_por_zero:
    mov rsi, erro_msg  ; Carrega o endereço da mensagem de erro
    call print_string
